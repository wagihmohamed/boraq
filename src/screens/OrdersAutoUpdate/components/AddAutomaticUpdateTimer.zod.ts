...
export const orderStatusAutomaticUpdateCreateSchema = z
  .object({
    orderStatus: z
      .string({
  ...
  .refine(
    ({ updateAt }) => {
      if (updateAt === 0) return false;
      return true;
    },
    {
      message: 'يجب ادخال وقت التحديث',
      path: ['updateAt'],
    }
  )
  .refine(
    ({ updateAt }) => {
      if (updateAt > 24) return false;
      return true;
    },
    {
      message: 'لا يمكن ان يكون وقت التحديث اكثر من 24 ساعة',
      path: ['updateAt'],
    }
  )
  .refine(
    ({ checkAfter }) => {
      if (checkAfter === 0) return false;
      return true;
    },
    {
      message: 'يجب ادخال وقت التحديث',
